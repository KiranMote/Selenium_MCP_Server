...
async function logAction(action, success, duration, details = {}) {
    const log = {
...
async function getElementWithEnhancedError(selector, method, timeout) {
...
async function generateElementSuggestions(selector, method) {
    const suggestions = [];
    if (!driver)
        return suggestions;
    try {
        ...
        `);
                const similarTexts = allText.filter(text => text.toLowerCase().includes(textToFind.toLowerCase()) ||
                    textToFind.toLowerCase().includes(text.toLowerCase())).slice(0, 3);
...
async function getAvailableElements() {
    if (!driver)
        return [];
    try {
